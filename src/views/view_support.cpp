...
static int call_in_thread(CTC_FUNC func, void* param) {
  ...
  if (view_thread == NULL) {
    ...
    catch(std::bad_alloc&) { error("E failed to allocate structure for view thread"); }
    int err = pthread_create(&new_thread_info->thread, NULL, view_thread_func, new_thread_info);
    if (err) {
      delete new_thread_info;
      error("E failed to create main thread, error: %d", err);
    }
...
static int add_view_in_thread(void* view_pars_ptr)
{
...
static int remove_view_in_thread(void* remove_params_ptr)
{
...
static void on_display_stub(void) { STUB_CALL( wnd->pre_display() ); }
static void on_reshape_stub(int width, int height) { STUB_CALL( wnd->on_reshape(width, height) ); }
static void on_mouse_move_stub(int x, int y) { STUB_CALL( wnd->on_mouse_move(x, y) ); }
static void on_key_down_stub(unsigned char key, int x, int y) { STUB_CALL( wnd->on_key_down(key, x, y) ); }
static void on_special_key_stub(int key, int x, int y) { STUB_CALL( wnd->on_special_key(key, x, y) ); }
static void on_entry_stub(int state) { STUB_CALL( wnd->on_entry(state) ); }
static void on_mouse_click_stub(int button, int state, int x, int y) {
  STUB_GET_VIEW();
  if (wnd == NULL)
    return;
