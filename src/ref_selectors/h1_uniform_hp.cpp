...
namespace RefinementSelectors {
  ...
  void H1UniformHP::calc_projection_errors(Element* e, const int max_quad_order_h, const int max_quad_order_p, const int max_quad_order_aniso, Solution* rsln, SonProjectionError herr[4], SonProjectionError anisoerr[4], SonProjectionError perr) {
    assert_msg(get_h_order(max_quad_order_h) <= H2DRS_MAX_ORDER && get_v_order(max_quad_order_h) <= H2DRS_MAX_ORDER, "Maximum allowed order of a son of H-candidate is %d but order (H:%d,V:%d) requested", H2DRS_MAX_ORDER, get_h_order(max_quad_order_h), get_v_order(max_quad_order_h));
    assert_msg(get_h_order(max_quad_order_p) <= H2DRS_MAX_ORDER && get_v_order(max_quad_order_p) <= H2DRS_MAX_ORDER, "Maximum allowed order of a son of P-candidate is %d but order (H:%d,V:%d) requested", H2DRS_MAX_ORDER, get_h_order(max_quad_order_p), get_v_order(max_quad_order_p));
    assert_msg(get_h_order(max_quad_order_aniso) <= H2DRS_MAX_ORDER && get_v_order(max_quad_order_aniso) <= H2DRS_MAX_ORDER, "Maximum allowed order of a son of ANISO-candidate is %d but order (H:%d,V:%d) requested", H2DRS_MAX_ORDER, get_h_order(max_quad_order_aniso), get_v_order(max_quad_order_aniso));
    assert_msg(get_h_order(max_quad_order_h) == get_v_order(max_quad_order_h), "Maximum orders (H:%d,V:%d) of H-candidate is in not uniform", get_h_order(max_quad_order_h), get_v_order(max_quad_order_h));
    assert_msg(get_h_order(max_quad_order_p) == get_v_order(max_quad_order_p), "Maximum orders (H:%d,V:%d) of P-candidate is in not uniform", get_h_order(max_quad_order_p), get_v_order(max_quad_order_p));
    assert_msg(get_h_order(max_quad_order_aniso) == get_v_order(max_quad_order_aniso), "Maximum orders (H:%d,V:%d) of ANISO-candidate is in not uniform", get_h_order(max_quad_order_aniso), get_v_order(max_quad_order_aniso));
...
#define make_ani_cand(q0, q1, iso) candidates.push_back(Cand(iso, make_quad_order(q0, q0), make_quad_order(q1, q1)))
  ...
  void H1UniformHP::update_shared_mesh_orders(const Element* element, const int orig_quad_order, const int refinement, int tgt_quad_orders[H2D_MAX_ELEMENT_SONS], const int* suggested_quad_orders) {
    assert_msg(get_v_order(orig_quad_order) == 0 || get_h_order(orig_quad_order) == get_v_order(orig_quad_order), "Non-uniform orig_quad_order (H:%d,V:%d) not support for this selector", get_h_order(orig_quad_order), get_v_order(orig_quad_order));
