...
void HcurlOrthoHP::get_optimal_refinement(Element* e, int order, Solution* rsln, int& split, int p[4], bool aniso, bool h_adapt)
{
  int i, j, k, n = 0;
  ...
  int max_order = std::min(6, (20 - e->iro_cache)/2 - 1);
...
void HcurlOrthoHP::adapt(double thr, int strat, bool aniso)
{
  if (!have_errors)
  ...
  for (i = 0; i < nact; i++)
  {
    ...
    if (!h_only)
      get_optimal_refinement(e, current, rsln[comp], split, p, aniso);
    else
